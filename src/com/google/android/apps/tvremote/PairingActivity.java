...
import com.doubtech.livewatch.googletvremote.CoreServiceHandler;
...
public class PairingActivity extends Activity implements
     ...
     private AlertDialog createPairingDialog(final PairingClientThread client) {
          ...
          final EditText pinEditText = (EditText) view
                    .findViewById(R.id.pairing_pin_entry);

          builder.setPositiveButton(R.string.pairing_ok,
                    ...
                    .setMessage(remoteDevice.getName()).setView(view);
     ...
     private final class PairingClientThread extends Thread {
          ...
          public void run() {
               ...
               try {
                    ...
                    try {
                         socketFactory = DummySSLSocketFactory
                                   .fromKeyManagers(mCoreServiceHandler
                                             .getKeyStoreManager().getKeyManagers());
                    } catch (GeneralSecurityException e) {
                         throw new IllegalStateException(
                                   "Cannot build socket factory", e);
                    }
                    ...
                    PoloWireInterface protocol = WireFormat.PROTOCOL_BUFFERS
                              .getWireInterface(context);
                    ClientPairingSession pairingSession = new ClientPairingSession(
                              protocol, context, "AnyMote", REMOTE_NAME);
                    ...
                    PairingListener listener = new PairingListener() {
                         ...
                         public void onPerformOutputDeviceRole(
                                   PairingSession session, byte[] gamma) {
                              Log.d(LOG_TAG,
                                        "onPerformOutputDeviceRole: "
                                                  + session
                                                  + ", "
                                                  + session.getEncoder().encodeToString(
                                                            gamma));
                         }
                         ...
                         public void onPerformInputDeviceRole(PairingSession session) {
                              ...
                              if (!isCancelling && secret != null) {
                                   try {
                                        byte[] secretBytes = session.getEncoder()
                                                  .decodeToBytes(secret);
                                        session.setSecret(secretBytes);
                                   } catch (IllegalArgumentException exception) {
                                        Log.d(LOG_TAG,
                                                  "Exception while decoding secret: ",
                                                  exception);
                                        session.teardown();
                                   } catch (IllegalStateException exception) {
                                        // ISE may be thrown when session is currently
                                        // terminating
                                        Log.d(LOG_TAG,
                                                  "Exception while setting secret: ",
                                                  exception);
                                        session.teardown();
                                   }
                              } else {
                    ...
                    if (ret) {
                         ...
                         mCoreServiceHandler.getKeyStoreManager().storeCertificate(
     ...
     private ProgressDialog buildProgressDialog() {
          ...
          dialog.setOnKeyListener(new DialogInterface.OnKeyListener() {
               public boolean onKey(DialogInterface dialogInterface, int which,
                         KeyEvent event) {
